...
impl<'a> PL061<'a> {
    ...
    fn mask(&self, pin_id: usize) -> Result<u8, Error> {
        if pin_id > PIN_COUNT { return Err(Error::InvalidPinId) };
        Ok(1 << pin_id)
    }
    ...
    fn mut_data_ptr(&'a mut self, pin_id: usize) -> Option<UniqueMmioPointer<'a, ReadPureWrite<u8>>> {
    ...
    pub fn pin_set_high(&'a mut self, pin_id: usize) -> Result<(), Error> {
        ...
        let Some(mut ptr) = self.mut_data_ptr(pin_id) else { return Err(Error::InvalidPinId) };
        ptr.write(0xFF);
        Ok(())
    }
    ...
    pub fn pin_is_high(&'a self, pin_id: usize) -> Result<bool, Error> {
        let Some(ptr) = self.data_ptr(pin_id) else { return Err(Error::InvalidPinId) };
