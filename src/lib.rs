...
impl<'a> PL061<'a> {
    ...
    pub fn into_output(&mut self, pin_id: usize) -> Result<(), InvalidPinError> {
        let mask = Self::mask(pin_id)?;
        let mut gpiodir_ptr = field!(self.regs, gpiodir);
        let val = gpiodir_ptr.read();
        gpiodir_ptr.write(val | mask);
    ...
    pub fn set_interrupt_event_rising_high(&mut self, pin_id: usize) -> Result<(), InvalidPinError> {
        let mask = Self::mask(pin_id)?;
    ...
    pub fn set_interrupt_event_falling_low(&mut self, pin_id: usize) -> Result<(), InvalidPinError> {
        let mask = Self::mask(pin_id)?;
        let mut ptr = field!(self.regs, gpioiev);
        let val = ptr.read();
        ptr.write(val & !mask);
    ...
    /// Drives the pin high.
    pub fn pin_set_high(&mut self, pin_id: usize) -> Result<(), InvalidPinError> {
    ...
    /// Drives the pin low.
    pub fn pin_set_low(&mut self, pin_id: usize) -> Result<(), InvalidPinError> {
...
mod tests {
    ...
    fn pin_id_checks() {
        ...
        assert_eq!(pl061.pin_is_high(8), Err(InvalidPinError));
        assert_eq!(pl061.pin_is_low(8), Err(InvalidPinError));
    }
